...
class ApiTestCase(unittest.TestCase):
    ...
    def test_search(self):
        ...
        os.environ["TRGM_FEATURE_QUERY"] = "UNION ALL".join(
            list(
                map(
                    lambda x: """
            SELECT
        ...
        self.assertEqual(
            len(
                list(
                    filter(lambda rc: rc["filterword"] == "Test", data["result_counts"])
                )
            ),
            1,
        )
        self.assertEqual(
            len(
                list(
                    filter(lambda rc: rc["filterword"] == "Map", data["result_counts"])
                )
            ),
            1,
        )
        ...
        features = list(
            filter(lambda result: result.get("feature", None), data["results"])
        )
        ...
        dataproducts = list(
            filter(lambda result: result.get("dataproduct", None), data["results"])
        )
    ...
    def test_search_templates(self):
        ...
        os.environ["SEARCH_BACKEND"] = "trgm"
        os.environ["TRGM_FEATURE_QUERY_TEMPLATE"] = "UNION ALL".join(
            list(
                map(
                    lambda x: """
            SELECT
