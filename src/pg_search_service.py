...
class PgClient:
    ...
    def search(self, identity, searchtext, searchfilter, limit):
        ...
        search_ds = list(
            filter(
                lambda facet: facet not in ["foreground", "background"], search_facets
            )
        )
        search_dp = list(
            filter(lambda facet: facet in ["foreground", "background"], search_facets)
        )
        ...
        with self.db_engine.db_engine(self.db_url).connect() as conn:
            conn.execute(
                sql_text("SET pg_trgm.similarity_threshold = :value"),
            ...
            if search_dp and layer_query:
                ...
                layer_results = (
                    conn.execute(
                        sql_text(layer_query),
                        {
                            "term": " ".join(tokens),
                            "terms": tokens,
                            "thres": self.similarity_threshold,
                            "facets": search_dp,
                        },
                    )
                    .mappings()
                ...
                if stacktype:
                    if not stacktype in result_counts:
                        result_counts[stacktype] = {
                            ...
                            "filterword": self.facets.get(stacktype, {}).get(
                                "filter_word", stacktype
                            ),
                            "count": 0,
                        }
            ...
            if feature_result["facet_id"] in search_ds:
                if not feature_result["facet_id"] in result_counts:
                    result_counts[feature_result["facet_id"]] = {
                        ...
                        "filterword": self.facets.get(
                            feature_result["facet_id"], {}
                        ).get("filter_word", feature_result["facet_id"]),
                        "count": 0,
                    }
                feature_result_count += 1
                result_counts[feature_result["facet_id"]]["count"] += 1
                if (
                    feature_result_count <= limit
                    and result_counts[feature_result["facet_id"]]["count"]
                    <= self.facet_search_limit
                ):
                    results.append(
                        {
                            "feature": {
                                ...
                                "id_field_type": feature_result.get(
                                    "id_in_quotes", None
                                ),
